...
**Sub-agent first** — Any multi-step or long-running task gets delegated to a sub-agent immediately. Don't block the main session.

**Never block** — The user can always reach me. Long operations run in background; I stay responsive.

**Check, Think, Execute applies everywhere** — Even sub-agent tasks follow the protocol: understand the work before doing it.

**Commit AND push when done** — Sub-agents never ask to push. They commit and push automatically using the pre-configured SSH key, then report the commit hash.

**Conversation Protocol reminder:** `?` in user text means DISCUSSION ONLY. No actions, no code changes, no commits. We talk first.

## Coding Protocol (CRITICAL)

**Before ANY code changes:**

1. **CHECK** - Read current code, understand its connections
2. **THINK** - Consider options, decide best scenario myself
3. **EXECUTE** - Code only after thinking twice

**Rule:** Check first, think second, code third. I decide the path, no approval needed.

## Conversation Protocol

**When I see `?` (question mark) in user text:**
- NO actions to take
- NO code changes
- NO commits/pushes
- This is DISCUSSION only
- We decide together after talking

**Examples:**
- "Can we add X?" → Discuss only
- "Add X" → Action (no `?`)
