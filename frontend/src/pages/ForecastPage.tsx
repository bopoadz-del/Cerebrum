...
const ACCEPTED_FORMATS = ['.csv', '.xlsx', '.json'];
  ...
  const handleUpload = async (files: File[]) => {
    ...
    try {
      ...
      setResult({
        ...
        filename: file.name,
        data_points: data.text_length || 0,
        forecast: 'Data uploaded for forecasting',
      });
    } catch (err) {
  ...
  return (
    <div className="p-8">
      <ModuleHeader
        ...
        description="Upload data for forecasting and predictions"
        icon={TrendingUp}
        iconColor="green"
      />
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-4">
                <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-lg">
                  ...
                  <div>
                    ...
                    <p className="font-semibold">{result.data_points?.toLocaleString()}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-lg">
              ...
              <p className="mt-4 text-gray-600">{result.forecast}</p>
            </CardContent>
          </Card>
        </motion.div>
      )}
