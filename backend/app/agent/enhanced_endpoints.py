"""
Agent API - AI Implementation
AI agent endpoints for chat and execution using OpenAI/Claude
"""
...
from app.services.ai_service import get_ai_service, generate_chat_response, AIService

logger = get_logger(__name__)
router = APIRouter(prefix="/agent", tags=["agent"])

# Agent system prompt
AGENT_SYSTEM_PROMPT = """You are Cerebrum AI, a construction intelligence assistant powered by advanced AI.
...
When asked about costs, provide ranges with context about location factors and market conditions."""
...
class AgentChatRequest(BaseModel):
    ...
    model: Optional[str] = Field(None, description="Model to use (gpt-4o-mini, gpt-4o, claude-3-5-sonnet)")
...
async def get_agent_status():
    """Get agent status and available models."""
    ...
    return {
        ...
        "version": "2.0.0",
        "capabilities": ["chat", "code", "analysis", "documents"],
...
async def chat_completion(
    ...
    """
    AI chat completion with conversation persistence.
    
    This endpoint:
    1. Creates or retrieves a conversation
    2. Loads previous messages for context
    3. Sends to AI for response
    4. Saves the exchange to database
    """
    service = get_ai_service()
