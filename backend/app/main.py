...
from app.api.deps import get_user_id_for_rate_limit  # User-based rate limiting
from app.agent.enhanced_core import initialize_agent
...
# Global rate limiter with default limits - uses user ID when available, falls back to IP
limiter = Limiter(
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.info("EnhancedCerebrumAgent pre-initialized")
    except Exception as e:
        logger.warning("Agent pre-initialization failed (will retry on first request)", error=str(e))
        # Don't exit - agent will retry on first request
...
def create_application() -> FastAPI:
    ...
    # CORS Error Handler - ensures CORS headers on all error responses
    @app.middleware("http")
    async def cors_error_handler(request: Request, call_next):
        """Add CORS headers to error responses to prevent browser CORS errors masking real issues."""
        try:
...
# Redirect to WebSocket test page for easy debugging
@app.get("/ws-test")
async def websocket_test_redirect():
    """Redirect to WebSocket test page."""
    from fastapi.responses import RedirectResponse
    return RedirectResponse(url="/static/websocket_test.html")
...
# Deploy trigger Thu Apr  2 06:20:43 PM CST 2026
# Deploy Fri Apr  3 03:35:42 AM CST 2026
