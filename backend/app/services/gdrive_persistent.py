...
class PermanentGoogleDrive:
    ...
    async def ensure_fresh_token(self) -> bool:
        ...
        try:
            ...
            if creds:
                logger.info(f"Token valid for user {self.user_id}")
                return True
            
            # Token expired or not found - try to refresh
            logger.info(f"Token needs refresh for user {self.user_id}, attempting...")
            ...
            logger.error(f"Error in ensure_fresh_token: {e}", exc_info=True)
            return False
