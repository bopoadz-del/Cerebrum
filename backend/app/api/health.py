...
@router.get("/embeddings", status_code=status.HTTP_200_OK)
async def embeddings_health() -> Dict[str, Any]:
    """ML embeddings model health check.
    
    Returns status of the sentence-transformers model.
    """
    from app.services.chroma_service import get_chroma_service
    
    chroma = get_chroma_service()
    embedding_info = chroma.get_embedding_model_info()
    ...
    return {
        ...
        "using_ml": embedding_info["using_ml"],
        "available": embedding_info["available"],
        "chromadb_ready": chroma.is_ready(),
    }
