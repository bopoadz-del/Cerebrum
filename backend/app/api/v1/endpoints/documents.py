...
# Optional auth helper for public uploads
...
# Public File Upload Endpoint (for frontend file attachments)
@router.post("/upload/public")
async def upload_public_file(
    file: UploadFile = File(...),
    current_user: Optional[User] = Depends(get_current_user_optional)
) -> Dict[str, Any]:
    """Upload a file via public/chat interface.
    
    Similar to /upload/chat but with optional authentication.
    Stores the file and optionally extracts text for indexing.
    Returns file metadata including URL for access.
    """
    import uuid
    ...
    try:
        ...
        file_id = f"{user_id}_{uuid.uuid4().hex}"
        file_ext = os.path.splitext(file.filename)[1].lower()
        safe_filename = f"{file_id}{file_ext}"
        file_path = os.path.join(UPLOAD_DIR, safe_filename)
        ...
        if file_ext in text_supported_exts or mime_type.startswith("text/"):
            try:
                ...
                # Index in ChromaDB if text was extracted
                if extracted_text and len(extracted_text) > 50 and current_user:
                    chroma = get_chroma_service()
                    doc_id = f"public_upload_{file_id}"
                    metadata = {
                        'name': file.filename,
        ...
        logger.error(f"Public file upload failed: {e}")
        raise HTTPException(status_code=500, detail=f"Upload failed: {str(e)}")
